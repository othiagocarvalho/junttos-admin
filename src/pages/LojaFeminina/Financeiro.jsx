  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 15, fontWeight: 700, color: 'var(--ink)' }}>Contas a Pagar</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 12px' }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 19, fontWeight: 700, color: '#ca8a04' }}>{fmtR(totalPendente)}</p>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 12px' }}>
          <p style={{ ...labelStyle, marginBottom: 6 }}>Atrasado</p>
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 19, fontWeight: 700, color: '#ef4444' }}>{fmtR(totalAtrasado)}</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {['todas', 'pendente', 'atrasado', 'pago'].map(f => (
          <button key={f} onClick={() => setFiltro(f)} style={{ padding: '6px 12px', borderRadius: 99, border: filtro === f ? 'none' : '1px solid var(--line)', background: filtro === f ? theme.primary : 'var(--surface)', color: filtro === f ? '#fff' : 'var(--muted)', fontFamily: 'Manrope, sans-serif', fontSize: 12, fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize' }}>
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--muted)', fontFamily: 'Manrope, sans-serif' }}>Carregando...</div>
      ) : filtradas.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, padding: '40px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, color: 'var(--muted)' }}>Nenhuma conta encontrada</p>
        </div>
      ) : (
              ...
              <div key={c.id} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
                    <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color: 'var(--ink)' }}>{fmtR(c.valor)}</p>
                ...
                {c._status !== 'pago' && (
                  <button onClick={() => handlePagar(c.id)} disabled={pagandoId === c.id} style={{ width: '100%', height: 36, borderRadius: 10, border: 'none', background: pagandoId === c.id ? 'var(--line)' : '#16a34a', color: '#fff', cursor: 'pointer', fontFamily: 'Manrope, sans-serif', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    <Check size={13} /> {pagandoId === c.id ? 'Registrando...' : 'Marcar como pago'}
                  </button>
                )}
                {c._status === 'pago' && c.data_pagamento && (
                  <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 11, color: '#16a34a', textAlign: 'center', marginTop: 4 }}>Pago em {fmtDate(c.data_pagamento)}</p>
                )}
              </div>
            )
          })}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', borderRadius: '20px 20px 0 0', padding: '24px 20px 40px', width: '100%', maxWidth: 480, maxHeight: '90dvh', overflowY: 'auto', boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <p style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--ink)' }}>Nova Conta a Pagar</p>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
              <button onClick={() => setShowModal(false)} style={{ flex: 1, height: 46, borderRadius: 12, border: 'none', background: 'var(--bg)', cursor: 'pointer', fontFamily: 'Manrope, sans-serif', fontWeight: 600, color: 'var(--ink)' }}>Cancelar</button>
              <button onClick={handleSalvar} disabled={saving || !form.descricao || !form.valor || !form.data_vencimento} style={{ flex: 2, height: 46, borderRadius: 12, border: 'none', background: (!saving && form.descricao && form.valor && form.data_vencimento) ? theme.primary : 'var(--line)', cursor: 'pointer', fontFamily: 'Manrope, sans-serif', fontWeight: 700, color: '#fff' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        {[
          ...
          <div key={label} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 10px' }}>
            ...
            <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, fontWeight: 700, color }}>{fmtR(val)}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {['todas', 'pendente', 'atrasado', 'recebido'].map(f => (
          <button key={f} onClick={() => setFiltro(f)} style={{ padding: '6px 12px', borderRadius: 99, border: filtro === f ? 'none' : '1px solid var(--line)', background: filtro === f ? theme.primary : 'var(--surface)', color: filtro === f ? '#fff' : 'var(--muted)', fontFamily: 'Manrope, sans-serif', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
            {f === 'todas' ? 'Todas' : f.charAt(0).toUpperCase() + f.slice(1)}
          </button>
        ))}
      </div>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', borderRadius: '20px 20px 0 0', padding: '24px 20px 40px', width: '100%', maxWidth: 480, maxHeight: '90dvh', overflowY: 'auto', boxSizing: 'border-box' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <p style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--ink)' }}>Nova Conta a Receber</p>
              <button onClick={() => setShowModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted)', padding: 4 }}><X size={18} /></button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '10px 14px' }}>
        ...
        <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, fontWeight: 700, color: 'var(--ink)', textTransform: 'capitalize' }}>{periodo.label}</span>
        <button onClick={() => setPeriodo(p => navegarMes(p.inicio, 1))} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink)', display: 'flex', alignItems: 'center', padding: 4 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 10px' }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, fontWeight: 700, color: '#16a34a' }}>{fmtR(totalEntradas)}</p>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 10px' }}>
          <p style={{ ...labelStyle, fontSize: 9, marginBottom: 4 }}>Saídas</p>
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, fontWeight: 700, color: '#ef4444' }}>{fmtR(totalSaidas)}</p>
        </div>
        <div style={{ background: saldoFinal >= 0 ? 'rgba(22,163,74,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${saldoFinal >= 0 ? 'rgba(22,163,74,0.2)' : 'rgba(239,68,68,0.2)'}`, borderRadius: 14, padding: '12px 10px' }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, fontWeight: 700, color: saldoFinal >= 0 ? '#16a34a' : '#ef4444' }}>{fmtR(saldoFinal)}</p>
        </div>
      </div>
      ...
      {fluxo.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, padding: '40px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, color: 'var(--muted)' }}>Sem movimentações no período</p>
        </div>
      ) : (
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', overflowX: 'auto', paddingBottom: 8, minHeight: 80 }}>
            {fluxo.map(d => (
              <div key={d.data} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, flexShrink: 0, minWidth: 24 }}>
                ...
                <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 8, color: 'var(--muted)' }}>{d.data.slice(8)}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><div style={{ width: 10, height: 10, borderRadius: 2, background: '#16a34a' }} /><span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 11, color: 'var(--muted)' }}>Entradas</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><div style={{ width: 10, height: 10, borderRadius: 2, background: '#ef4444' }} /><span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 11, color: 'var(--muted)' }}>Saídas</span></div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {fluxo.map(d => (
            <div key={d.data} style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 12, fontWeight: 600, color: 'var(--ink)' }}>{fmtDate(d.data)}</p>
                <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 11, color: 'var(--muted)' }}>
                  +{fmtR(d.entradas)} / -{fmtR(d.saidas)}
                </p>
              </div>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 14, fontWeight: 700, color: d.saldo >= 0 ? '#16a34a' : '#ef4444' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ background: positivo ? 'rgba(22,163,74,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${positivo ? 'rgba(22,163,74,0.2)' : 'rgba(239,68,68,0.2)'}`, borderRadius: 16, padding: '20px 18px', textAlign: 'center' }}>
        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 10, fontWeight: 700, color: positivo ? '#16a34a' : '#ef4444', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 8 }}>Resultado Líquido</p>
        <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 36, fontWeight: 700, color: positivo ? '#16a34a' : '#ef4444', lineHeight: 1 }}>
          {positivo ? '+' : ''}{fmtR(dre.resultadoLiquido)}
        </p>
        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 12, color: positivo ? '#16a34a' : '#ef4444', marginTop: 6 }}>Margem: {dre.margemPercentual.toFixed(1)}%</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 12px' }}>
          ...
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#16a34a' }}>{fmtR(dre.receitaBruta)}</p>
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>Vendas no período</p>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 12px' }}>
          <p style={{ ...labelStyle, marginBottom: 6 }}>Total Despesas</p>
          <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: '#ef4444' }}>{fmtR(dre.totalDespesas)}</p>
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 10, color: 'var(--muted)', marginTop: 2 }}>Contas pagas</p>
        </div>
      </div>
      ...
      {dre.despesasPorCategoria.length > 0 ? (
              ...
              return (
                <div key={categoria}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, fontWeight: 700, color: theme.primary }}>{fmtR(total)}</span>
                  </div>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 16, padding: '32px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, color: 'var(--muted)' }}>Nenhuma despesa paga no período</p>
        </div>
      )}
    </div>
  )
