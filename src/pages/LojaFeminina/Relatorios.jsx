...
import { ChevronRight, ArrowLeft, Search, Tag, Calendar, User, Clock, Pencil, Trash2, Plus, X } from 'lucide-react'
...
const PGTOS_LIST = ['Pix', 'Dinheiro', 'Cartão de Crédito', 'Cartão de Débito']

// ── VendasDetalhadas sub-view ────────────────────────────────
  ...
  return (
    <div style={{ paddingTop: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 15, fontWeight: 700, color: '#fff' }}>
          Vendas Detalhadas
        </span>
        <span style={{ marginLeft: 'auto', fontFamily: 'Manrope, sans-serif', fontSize: 12, color: 'rgba(255,255,255,0.75)' }}>
                ...
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <select
                    ...
                    style={{
                      ...
                      border: '1.5px solid var(--line)', borderRadius: 12, padding: '0 8px',
                      fontFamily: 'Manrope, sans-serif', fontSize: 12, fontWeight: 600,
                      color: 'var(--ink)', background: 'var(--bg)', outline: 'none', cursor: 'pointer', boxSizing: 'border-box',
                    }}
                  >
                    {PGTOS_LIST.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                  <div style={{ position: 'relative', flex: '1 1 0', minWidth: 0 }}>
                    ...
                    <input
                      ...
                      style={{
                        ...
                        padding: '0 10px 0 30px', fontFamily: 'Manrope, sans-serif', fontSize: 14, fontWeight: 700,
...
export default function Relatorios({ vendas, deleteVenda, updateVenda, theme }) {
  ...
  const totalFaturamento = filtered.reduce((s, v) => s + Number(v.valor), 0)
  const nVendas = filtered.length
  const ticketMedio = nVendas > 0 ? totalFaturamento / nVendas : 0
  const totalItens = filtered.reduce((s, v) => {
    return s + (v.produtos || []).reduce((q, p) => q + (p.quantidade || 1), 0)
  }, 0)
  const pa = nVendas > 0 ? totalItens / nVendas : 0

  const pgtoMap = useMemo(() => {
    const map = {}
    filtered.forEach(v => {
      parsePgtos(v).forEach(p => {
        map[p.forma] = (map[p.forma] || 0) + Number(p.valor)
      })
    })
    return map
  }, [filtered])
  ...
  const kpis = [
    { label: 'Faturamento',  value: fmtR(totalFaturamento) },
    { label: 'Nº de Vendas', value: nVendas },
    { label: 'Ticket Médio', value: fmtR(ticketMedio) },
    { label: 'P.A.',         value: nVendas > 0 ? pa.toFixed(1) : '—', sub: 'peças/atend.' },
  ]

  return (
    <div style={{ paddingTop: 8 }}>
      {/* Header */}
      <div style={{ background: theme.primary, borderRadius: 16, padding: '18px 20px', marginBottom: 16 }}>
        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.7)', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 4 }}>
          Relatórios
        </p>
        <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700, color: '#fff', lineHeight: 1 }}>
          {fmtR(totalFaturamento)}
        </p>
        <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 12, color: 'rgba(255,255,255,0.7)', marginTop: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
        {kpis.map(({ label, value, sub }) => (
          <div key={label} style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--line)', padding: '16px 14px' }}>
            <p style={{ fontFamily: 'Manrope, sans-serif', fontSize: 10, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 8 }}>{label}</p>
            <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: 'var(--ink)', lineHeight: 1, marginBottom: sub ? 4 : 0 }}>{value}</p>
            {sub && <p style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'Manrope, sans-serif' }}>{sub}</p>}
          </div>
        ))}
      </div>

      {/* Formas de pagamento */}
      <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--line)', padding: '20px 18px', marginBottom: 12 }}>
        ...
        {Object.keys(pgtoMap).length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {Object.entries(pgtoMap).sort((a, b) => b[1] - a[1]).map(([pgto, val]) => {
              const pct = totalFaturamento > 0 ? (val / totalFaturamento) * 100 : 0
              return (
                <div key={pgto}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, color: 'var(--ink)' }}>{pgto}</span>
                    <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, fontWeight: 700, color: 'var(--rose-deep)' }}>{fmtR(val)}</span>
                  </div>
